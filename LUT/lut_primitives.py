...
def n_current_mirror(id_0, vds_1, fins_0, fins_1, len_01, tb):
    ...
    cdd_0 = tb.lookup(par="cdd", len_val=len_01, fin_val=fins_0, type="n", vds_val = vds_0, vgs_val=vgs_0)
    cdd_1 = tb.lookup(par="cdd", len_val=len_01, fin_val=fins_1, type="n", vds_val = vds_1, vgs_val=vgs_1)


    m0 = device(vgs_0, vds_0, 0, fins_0, len_01, id=id_0, cdd = cdd_0 )
    m1 = device(vgs_1, vds_1, 0, fins_1, len_01, id=id_1, cdd = cdd_1 )

    return m0, m1


def n_differential_pair(id_s, vd_0, vd_1, fins_01, len_01, v_diff,  v_cm, tb):
    # vds_0, vgs_0 ------------------------> m0
    # vds_1, vgs_1 ------------------------> m1
...
def p_current_mirror(vdd, id_0, vd_1, fins_01, len_01, tb):
...
def n_lv_cascode_cm(vd0, vd1, vbias, fins_01, fins_23, len_01, len_23, tb):
    assert vbias > vd0/2, "the bias voltage is too small"
    vx = vd0/2 # initial guess
    VDS = np.linspace(0, 0.8, 41)

    for i in range(10):
        id_2 = tb.lookup(par="id", len_val=len_23, fin_val=fins_23, type="n", vgs_val=vbias-vx, vds_val=vd0-vx)
        id_0_ = tb.lookup(par="id", len_val=len_01, fin_val=fins_01, type="n", vgs_val=vd0)

        f = interp1d(id_0_, VDS)
        v0 = f(id_2)
