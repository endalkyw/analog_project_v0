...
class lut:
    def __init__(self, vgs: np.array([float]), vds: np.array([float]), vsb: np.array([float]),
        ...
        self.params = ['gm', 'cdd', 'vth', 'gds', 'id', 'cds', 'cgg', 'cgd', 'cgs']
    ...
    def read_lut(self):
        ...
        for ind in range(len(df_n)):
            n_line = df_n[ind][:]
            p_line = df_p[ind][:]
            # gm, cdd, vth, gds, id, cds, cgg, cgd, cgs,
            for i, name in enumerate(self.params):
                ...
                self.get_index(self.vds, n_line[2]), self.get_index(self.vsb, n_line[1]),
                ...
                self.get_index(self.vds, p_line[2]), self.get_index(self.vsb, p_line[1]),
                self.get_index(self.vgs, p_line[0])] = p_line[i + 5]

    def get_index(self, array, element):
        return np.where(np.isclose(array, element))[0][0]
