...
class Five_T_OTA():
    ...
    def design_ota(self, TE, beta, vcm, vds_1, length):
        cdd_1 = 0
        cdd_2 = 0
        gm_id_0, gm_id_1, gm_id_2, gm_id_3 = TE
        for _ in range(10):  # DP and Upper PMOS CM
            ...
            for _ in range(10):
                vsg_3 = vsg_2
                vsd_3 = self.supp_V - (vx + vds_1) # guess the vsd_3 value
                id_3 = -1 * self.tb.lookup("id", length[2], self.fin_ref, "p", vgs_val = vsg_3,  vds_val=vsd_3)*(fins_3/self.fin_ref)

                # Find the actual vds from the bottom diode connected nmos:
                for _ in range(5):  # this is for the PMOS
                    gm_3_ = self.tb.lookup("gm", length[3], self.fin_ref, "n", vds_val=self.supp_V - vsd_3)
                    id_3_ = self.tb.lookup("id", length[3], self.fin_ref, "n", vds_val=self.supp_V - vsd_3)
                    gm_id_3_ = gm_3_ / id_3_
                    vgs_4 = self.interpolate_array(VGS, gm_id_3_, gm_id_3)
                    vgs_4 = vds_4
                    vsd_3 = self.supp_V - vds_4

            id_4 = id_3
            fins_4 = id_4 / self.tb.lookup("id", length[3], self.fin_ref, "n", vds_val=vds_4, vgs_val=vgs_4)*self.fin_ref
        ...
        fins = [fins_0[0], fins_1[0], fins_2[0], fins_3[0], fins_4[0]]
        #
        # try:
...
def main():
    ...
    fins, Is = ota.design_ota([18.5, 23.4, 17.3, 15], 2, 0.6, 0.4, [1, 1, 1, 1])
